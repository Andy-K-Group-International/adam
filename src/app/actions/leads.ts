...
async function sendNdaInviteEmail({ name, email, company }: { name: string; email: string; company: string }) {
  ...
  const html = wrap(`
    <h1 style="font-family:Georgia,serif;font-size:22px;font-weight:700;color:#0E282D;margin:0 0 20px;">Your A.D.A.M. demo application has been approved</h1>
    ...
    <div style="background:#f0f4f4;border-left:2px solid #2F9E9A;padding:16px 20px;border-radius:0 8px 8px 0;margin-bottom:28px;">
      ...
      <p style="color:#0E282D;font-size:14px;font-weight:600;margin:0;">Sign the NDA to unlock your private demo</p>
    </div>
    <a href="${ndaUrl}" style="display:inline-block;background:#2F9E9A;color:#fff;text-decoration:none;padding:12px 28px;border-radius:8px;font-size:14px;font-weight:600;margin-bottom:32px;">Sign NDA &amp; Access Demo &#8594;</a>
...
async function sendDemoRejectionEmail({ name, email, company, reason }: { name: string; email: string; company: string; reason?: string }) {
  const html = wrap(`
    <h1 style="font-family:Georgia,serif;font-size:22px;font-weight:700;color:#0E282D;margin:0 0 20px;">Thank you for your interest in A.D.A.M.</h1>
    <p style="color:#525a70;font-size:15px;line-height:1.7;margin:0 0 16px;">Hi ${name},</p>

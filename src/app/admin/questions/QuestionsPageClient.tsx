...
import { toggleQuestionActive, refreshQuestions } from "@/app/actions/questions";
...
export default function QuestionsPageClient({ initialQuestions, initialSections }: Props) {
  const [allQuestions, setAllQuestions] = useState<QuestionData[]>(initialQuestions);
  const [allSections] = useState<SectionData[]>(initialSections);
  ...
  const handleToggleActive = async (questionId: string, is_active: boolean) => {
    await toggleQuestionActive(questionId, !is_active);
    const result = await refreshQuestions();
    if (result.questions) {
      setAllQuestions(result.questions as QuestionData[]);
    }
  };
